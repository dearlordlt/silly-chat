...
def store_export(
    ...
    dk: bytes | None = None, kind: str = "export", gen_meta: str = "",
) -> str:
    """Store generated bytes in the uploads store (owner-only, sealed under the
    owner's data key when available). kind="export" is TTL-swept; kind="genimage"
    persists for the user's Gallery until deleted. Returns the id."""
    ...
    up = Upload(
        ...
        kind=kind, mime=mime, ext=ext, size=len(data), name=name,
        enc=1 if dk is not None else 0, gen_meta=gen_meta,
    )
    path = _path(up)
